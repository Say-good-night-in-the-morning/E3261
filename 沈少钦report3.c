...
int main()
{
	...
	u_int netmask;
	char packet_filter[] = "MACaddress and IPaddress";
...
void packet_handler(u_char *param, const struct pcap_pkthdr *header, const u_char *pkt_data)
{
	...
	/* set MAC address*/
	mac_address macadd;
	strncpy(macadd.byte, pkt_data, 12);
	...
	printf("%02X-%02X-%02X-%02X-%02X-%02X,%d.%d.%d.%d,%02X-%02X-%02X-%02X-%02X-%02X,%d.%d.%d.%d\n",
		macadd.byte[0], macadd.byte[1], macadd.byte[2], macadd.byte[3], macadd.byte[4], macadd.byte[5],
		ih->saddr.byte1, ih->saddr.byte2, ih->saddr.byte3, ih->saddr.byte4,
		macadd.byte[6], macadd.byte[7], macadd.byte[8], macadd.byte[9], macadd.byte[10], macadd.byte[11],
